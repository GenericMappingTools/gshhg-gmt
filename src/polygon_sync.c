...
int main (int argc, char **argv) {
	...
	if (argc == 1) {
		fprintf (stderr, "usage: polygon_sync path-to-full.b\n");
		exit (EXIT_FAILURE);
	}
	...
	for (res = 0; res < 5; res++) {
		...
		if ((fp = fopen (file, "r")) == NULL) {
			fprintf (stderr, "polygon_sync: Cannot open file %s\n", file);
			exit (EXIT_FAILURE);
		}
		n_id[res] = 0;
		while (pol_readheader (&P[res][n_id[res]].h, fp) == 1) {
			P[res][n_id[res]].p = (struct LONGPAIR *) GMT_memory (VNULL, P[res][n_id[res]].h.n, sizeof (struct LONGPAIR), "polygon_sync");
			if (pol_fread (P[res][n_id[res]].p, P[res][n_id[res]].h.n, fp) != P[res][n_id[res]].h.n) {
				fprintf(stderr,"polygon_sync:  ERROR  reading %d points from file %s.\n", P[res][n_id[res]].h.n, file);
				exit (EXIT_FAILURE);
			}
			n_id[res]++;
		}
	...
	for (res = 0; res < 5; res++) {	/* Initialize all parents to NONE */
		link[res] = (int *) GMT_memory (VNULL, n_id[FULL], sizeof (int), "polygon_sync");
		level[res] = (int *) GMT_memory (VNULL, n_id[FULL], sizeof (int), "polygon_sync");
		for (id1 = 0; id1 < n_id[FULL]; id1++) link[res][id1] = NOT_PRESENT;
	}
	
	if ((fp = fopen ("GSHHS_hierarchy.txt", "r")) == NULL) {
		fprintf (stderr, "polygon_sync: Cannot open hierarchy file\n");
		exit (EXIT_FAILURE);
	}
	for (id1 = 0; id1 < n_id[FULL]; id1++) {
		...
		for (res = 0; res < 5; res++) {
			if (res > 0 && link[res][id1] >= 0 && level[res][id1] != level[FULL][id1]) {
				fprintf (stderr, "ERROR: Pol %d (res = %c) has a different level in the full resolution\n", link[FULL][id1], kind[res]);
				error++;
				printf ("%s", line);
			}
			if (res > 0 && link[res][id1] >= 0 && link[res-1][id1] == -1) {
				fprintf (stderr, "ERROR: Siblings of pol %d (res = %c) missing in the next higher resolution\n", link[FULL][id1], kind[res]);
				error++;
				printf ("%s", line);
			}
		}
	...
	for (res = 1; res < 5; res++) {
		...
		if ((fp = fopen (file, "w")) == NULL) {
			fprintf (stderr, "polygon_sync: Cannot create file %s\n", file);
			exit (EXIT_FAILURE);
		}
		for (id2 = 0; id2 < n_id[res]; id2++) {
			if (id2 < N_CONTINENTS)
			...
			if (pol_fwrite (P[res][id2].p, P[res][id2].h.n, fp) != P[res][id2].h.n) {
				fprintf(stderr,"polygon_sync:  ERROR  writing %d points from file %s.\n", P[res][id2].h.n, file);
				exit (EXIT_FAILURE);
			}
		}
		fclose (fp);
