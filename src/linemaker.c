...
{
	int i, dims, kind, counts[16], old_fill_mode;
	...
	if (argc < 2) {
		fprintf (stderr, "usage: linemaker line_prefix [b|r] [-n]\n");
		fprintf (stderr, "	b = border, r = river [b]\n");
		fprintf (stderr, "	Use -n to build classic netCDF3 file\n");
		exit (EXIT_FAILURE);
	}
	...
	cmode = (!strcmp (argv[argc-1], "-n")) ? NC_CLASSIC_MODEL : NC_NETCDF4;
	...
	for (i = 0; i < s.n_bin; i++) {
		...
		if (fread ((void *)&bin_head, sizeof (struct GMT3_BIN_HEADER_LINE), (size_t)1, fp_bin) != 1) {
	...
	for (i = 0; i < s.n_seg; i++) {
		...
		if (fread ((void *)&seg_head, sizeof (struct SEGMENT_HEADER_LINE), (size_t)1, fp_seg) != 1) {
	...
	GMT_err_fail (nc_def_var_chunking (s.cdfid, s.seg_n_id, NC_CHUNKED, chunksize(s.n_seg)), file);
	GMT_err_fail (nc_def_var_deflate (s.cdfid, s.seg_n_id, 1, 1, 9), file);
	GMT_err_fail (nc_def_var (s.cdfid, "Hierarchial_level_of_a_segment", NC_SHORT, (size_t)1, &dims, &s.seg_level_id), file);
	GMT_err_fail (nc_def_var_chunking (s.cdfid, s.seg_level_id, NC_CHUNKED, chunksize(s.n_seg)), file);
	GMT_err_fail (nc_def_var_deflate (s.cdfid, s.seg_level_id, 1, 1, 9), file);
