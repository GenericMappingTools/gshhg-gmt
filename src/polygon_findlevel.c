...
int main (int argc, char **argv) {
	...
	while (pol_readheader (&blob[n_id].h, fp) == 1) {
		cont_no_1 = (blob[n_id].h.continent);		/* Get continent number 1-7 (0 if not a continent) */
	...
	for (id1 = 0; id1 < n_id; id1++) {	/* For all anchor polygons */
...
#endif
		cont_no_1 = (blob[id1].h.continent);	/* Get continent nubmer 1-7 (0 if not a continent) */
		if (Pol_Is_Antarctica (cont_no_1)) continue;	/* But skip Antarctica since there are no lakes in the data set */
		...
		for (id2 = 0; id2 < n_id; id2++) {
			...
			if (blob[id1].h.source == 2 && blob[id2].h.source == 3) continue;	/* Skip testing if ice islands are inside grounding line B&S (2007) */
			...
			if (full && cont_no_1 > 0 && !Pol_Is_Antarctica (cont_no_1)) {	/* Use course outlines to determine if id2 is inside/outside a continent */
				c = cont_no_1 - 1;
			...
			if (intest == 1) {	/* Should not happen - duplicate polygon? */
				...
				if (blob[id1].h.source == 0 && blob[id2].h.source == 0 && blob[id1].h.n == blob[id2].h.n) { /* duplicate */
					...
					continue;	/* Treat touching as not inside */
				else {
