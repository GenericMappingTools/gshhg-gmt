...
int main (int argc, char **argv)
{
	...
	int id, n_id, do_n, do_a, pos, k, sign, n_changed = 0, comp_blobs_a(), comp_blobs_n();
	double *flon = NULL, *flat = NULL;
	struct LONGPAIR pp;

	if (argc < 3) {
		fprintf (stderr,"usage:  polygon_sort old.b new.b [-a|A|n]\n");
		fprintf (stderr,"	-A will recalculate polygon areas\n");
		fprintf (stderr,"	-n will sort based on number of points\n");
		exit (-1);
	}
	...
	if (do_n)
		fprintf (stderr, "polygon_sort: Will sort based on number of vertices\n");
	else
	...
	fprintf (stderr, "polygon_sort: Done. %d polygons was given revised ID\n", n_changed);
	exit (0);
}
