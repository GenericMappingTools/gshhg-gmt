...
int main (int argc, char **argv)
{
	...
	if (argc == 1) {
		...
		fprintf(stderr,"	   bad.lis contains list of polygons to delete, or /dev/null if none\n");
		fprintf(stderr,"	   fix.lis contains list of polygons to replace with new ones, or /dev/null if none\n");
	...
	if (get_area) {
		area_init ();
		n_alloc = GMT_CHUNK;
		flon = (double *) GMT_memory ((void *)flon, n_alloc, sizeof(double), "polygon_update");
		flat = (double *) GMT_memory ((void *)flat, n_alloc, sizeof(double), "polygon_update");
	}
	n_id = 0;	
