 ...
 * the lower-resolution polygons. We doe this on the ASSUMPTION that
 * all sibling polygons shares at least one coordinate with the others
 * This is dangerous as if we make changes and this is no longer true
 * then we fail to find the siblings.
 */
#include "wvs.h"
	...
	for (id1 = 0; id1 < n_id[FULL]; id1++) {	/* For all full resolution polygons */
		...
		for (res = 1; res < 5; res++) {	/* For each of the lower resolutions */
			...
			for (id2 = 0; link[res][id1] == NOT_PRESENT && id2 < n_id[res]; id2++) {
				...
				cont_no_2 = (P[res][id2].h.continent);	/* Get continent nubmer 1-7 (0 if not a continent) */
				...
				ix_shift = lrint (x_shift) * MILL;
				if (cont_no_1 > 0 && !Pol_Is_Antarctica (cont_no_1)) {	/* Any continent other than Antarctica */
					c = cont_no_1 - 1;
				...
				/* OK, found a match, set father and link to end id2 loop */
