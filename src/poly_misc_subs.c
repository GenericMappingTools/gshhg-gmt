...
void crude_init (double *X[N_CONTINENTS][2], double *Y[N_CONTINENTS][2], int N[N_CONTINENTS][2])
{
	...
	int c, i, t;
	char line[BUFSIZ], *cont[N_CONTINENTS] = {"eur", "afr", "nam", "sam", "ant", "ant_g", "aus"}, *type[2] = {"out", "in"};
	double *x, *y;
	FILE *fp;
	
	for (c = 0; c < N_CONTINENTS; c++) {	/* For each continent */
		for (t = 0; t < 2; t++) {	/* For both outside and inside polygons */
			sprintf (line, "crude_%s_%s.txt", cont[c], type[t]);
...
void crude_init_int (int *IX[N_CONTINENTS][2], int *IY[N_CONTINENTS][2], int N[N_CONTINENTS][2], int scale)
{
	...
	for (c = 0; c < N_CONTINENTS; c++) {	/* For each continent */
		for (t = 0; t < 2; t++) {	/* For both outside and inside polygons */
			...
			for (i = 0; i < N[c][t]; i++) {
				//ix[i] = lrint (X[c][t][i]) * scale;
				//iy[i] = lrint (Y[c][t][i]) * scale;
				ix[i] = lrint (X[c][t][i] * scale);
				iy[i] = lrint (Y[c][t][i] * scale);
			}
			IX[c][t] = ix;
...
double area_size (double x[], double y[], int n, int *sign)
{
	...
	/* Geographic data, must use vector mean */
	memset ((void *)M, 0, (size_t)(3 * sizeof (double)));
	for (i = 0; i < n; i++) {
		GMT_geo_to_cart (y[i], x[i], P, TRUE);
		for (k = 0; k < 3; k++) M[k] += P[k];
	}
	GMT_normalize3v (M);
	GMT_cart_to_geo (&lat, &lon, M, TRUE);
